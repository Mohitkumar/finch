...
func (p *Picker) Pick(info balancer.PickInfo) (
	...
	if strings.Contains(info.FullMethodName, "Create") ||
		strings.Contains(info.FullMethodName, "Save") ||
		strings.Contains(info.FullMethodName, "Set") ||
		strings.Contains(info.FullMethodName, "Delete") ||
		len(p.followers) == 0 {
		result.SubConn = p.leader
		p.logger.Info("forwarding to leader")
	} else if strings.Contains(info.FullMethodName, "Get") {
		p.logger.Info("forwarding to folower")
